...
- **Always:** compute the realized non-overlapping period count `N = ⌊(len(in_window_dates) − delay − 1) / holding⌋ + 1`. If `N < 2`, reject (`WINDOW_TOO_SHORT`). Emit `N` into provenance and the `NON_OVERLAPPING_COHORTS` warning.
- **Fitted methods only:** a rebalance is *fittable* only once `ic_min_periods` prior periods have closed. Require the window to admit at least `ic_min_periods + 1` **fittable** rebalances (i.e. `N − ic_min_periods ≥ 1`). If **zero** rebalances would be fitted, **downgrade the method to `equal_weight` and set `is_fitted = false`** with a prominent `NO_FITTED_PERIODS` code (do not advertise `is_fitted=true` on an all-warmup run). If some but few are fitted, keep `is_fitted=true` and expose `fitted_period_fraction` (§4.4, §8).
