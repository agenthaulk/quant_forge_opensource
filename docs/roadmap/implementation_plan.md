...
| `fable/phase-c-falsification-battery` | `evaluation/falsification.py`: placebo/permutation, noise-injection IC retention, walk-forward IC decay; gate evidence wiring | none (kernel-side) | battery verdicts carry MetricValue statuses; gates block on configured-but-missing battery evidence (FP-2); **demo-data power policy: sample floors chosen so the demo panel can pass — an always-INSUFFICIENT battery is a lane failure, and demoting verdicts to advisory is forbidden** (B4 F7) |
...
| `fable/phase-c-server-decomposition` | apps/web/server.py → routing/api/html/jobs modules (characterization tests first) | none (rides M1) | no behavior change; route-by-route parity tests; **extraction completes BEFORE any run-timeline feature lands** (B4 F11) |
| `fable/phase-c-data-plane` | DataCatalogPort as a real port (replace the static field dict), field/operator expansion path, `visible_at` PIT-fundamentals contract (Studio-adopted), synthesized/capability wiring into the ValidationGate | none | gate's "data capability ready" is backed by the actual loaded catalog; unknown fields fail closed end-to-end (B4 F6/F7) |
