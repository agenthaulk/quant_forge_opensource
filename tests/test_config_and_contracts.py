...
def test_bootstrap_runtime_config_loads_runtime_env_files(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("QF_BOOTSTRAP_KEY", raising=False)
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / "local.env"
    env_path.write_text("QF_BOOTSTRAP_KEY=test-value\n", encoding="utf-8")
    config_path.write_text(
        """
runtime:
  env_files:
...
llm:
  ...
  providers:
    deepseek:
      ...
      api_key_env: QF_BOOTSTRAP_KEY
""",
        encoding="utf-8",
    )

    config = bootstrap_runtime_config(config_path)
...
""",
    ...
    assert custom_config.deduplication.result_precision == 4
    assert custom_config.deduplication.recent_trace_limit == 25
    assert custom_config.deduplication.max_same_shape_per_run == 1
