...
from quant_forge.factor_library.classification import (
    FACTOR_CATEGORY_DIRS,
    factor_category,
    is_factor_category_dir,
)
from quant_forge.factor_library.repository import FactorRepository

PRECOMPUTED_FORMULA_PREFIX = "precomputed:"
FACTOR_ID_STORE_PREFIX = "factor_id="
_PRECOMPUTED_FORMULA_RE = re.compile(
    rf"^{re.escape(PRECOMPUTED_FORMULA_PREFIX)}\s*([A-Za-z0-9_.=-]+)\s*$",
    re.IGNORECASE,
)
...
def normalize_precomputed_factor_store(
    ...
    *,
    ...
    for directory, metadata, factor in factors:
        ...
        target_dir = root / category_dir / target_key
        is_canonical = directory == target_dir
...
def precomputed_formula_store_key(formula: str) -> str:
    match = _PRECOMPUTED_FORMULA_RE.fullmatch(formula.strip())
    if match is None:
        return ""
...
def _normalized_factor_metadata(
    ...
    *,
    ...
    category_dir = FACTOR_CATEGORY_DIRS[factor_category(factor)]
    payload["factor_category"] = factor_category(factor)
    payload["factor_values_relative_path"] = f"{category_dir}/{target_key}"
    payload["storage_naming"] = "factor_category/factor_id_partition"
...
def _category_roots(root: Path) -> list[Path]:
