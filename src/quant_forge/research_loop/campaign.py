...
@dataclass(frozen=True)
class ResearchCampaignOptimizerMetadata:
    source: str = "local_campaign"
    provider: str = "rule"
...
class ResearchCampaignResult:
    ...
    optimizer: ResearchCampaignOptimizerMetadata = ResearchCampaignOptimizerMetadata()
    errors: tuple[str, ...] = ()
    rd_stage: str = RD_FACTOR_SYNTHESIS_STAGE
    workflow_type: str = "campaign"
...
class ResearchCampaignService:
    def __init__(
        ...
        *,
        ...
        precomputed_strategy_order: tuple[str, ...] = (),
        optimizer: ResearchCampaignOptimizerMetadata | None = None,
    ) -> None:
        ...
        self.precomputed_strategy_order = tuple(precomputed_strategy_order)
        self.optimizer = optimizer or ResearchCampaignOptimizerMetadata()
    ...
    def run(
        ...
        *,
        ...
        return ResearchCampaignResult(
            rd_stage=RD_FACTOR_SYNTHESIS_STAGE,
    ...
    def _evaluate_precomputed_candidate(
        ...
        *,
        ...
        strategy = self._precomputed_campaign_strategy(round_index)
        ...
        factor_dir = write_root.expanduser() / FACTOR_CATEGORY_DIRS[factor_category(candidate)] / f"factor_id={candidate.factor_id}"
    ...
    def _precomputed_campaign_strategy(self, round_index: int) -> _PrecomputedCampaignStrategy:
        if self.precomputed_strategy_order:
            suffix = self.precomputed_strategy_order[min(round_index - 1, len(self.precomputed_strategy_order) - 1)]
            strategies = {strategy.suffix: strategy for strategy in PRECOMPUTED_CAMPAIGN_STRATEGIES}
            if suffix not in strategies:
                raise ValueError(f"unsupported precomputed campaign strategy: {suffix}")
            base = strategies[suffix]
            return _PrecomputedCampaignStrategy(
                round_index=round_index,
                seed_limit=base.seed_limit,
                weighting=base.weighting,
                smoothing_span=base.smoothing_span,
                suffix=base.suffix,
            )
        return _precomputed_campaign_strategy(round_index)
