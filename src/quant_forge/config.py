...
class PathSettings:
    ...
    # Fundamental data plane (optional; both None -> price-volume-only panel).
    ...
    # ``qf data build-fundamentals`` reads it and writes the PIT overlay to
    # ``fundamentals_overlay_root``, which the panel loader then merges.
    fundamentals_source_root: Path | None = None
    fundamentals_overlay_root: Path | None = None
    ...
    def resolve(self, workspace: Path | None = None) -> "QuantForgeConfig":
        ...
        return QuantForgeConfig(
            paths=PathSettings(
                ...
                fundamentals_overlay_root=_optional_under(root, self.paths.fundamentals_overlay_root),
...
def load_config(config_path: Path | None = None, workspace: Path | None = None) -> QuantForgeConfig:
    ...
    config = QuantForgeConfig(
        paths=PathSettings(
            ...
            fundamentals_overlay_root=_optional_path_setting(raw, "fundamentals_overlay_root"),
...
def bootstrap_runtime_config(config_path: Path | None = None, workspace: Path | None = None) -> QuantForgeConfig:
    """Load runtime config and local env-files before any provider calls.

    This is the named entry point CLI/Web/agent surfaces should use. It makes
    the intended contract explicit: provider keys are referenced by config and
    loaded from ignored env-files or environment variables, never embedded in
    source code or public YAML.
    """
