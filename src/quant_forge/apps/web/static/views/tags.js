 ...
 * collapsed; absent frequency/warmup/horizon values render nothing (no
 * guessed defaults); a null tag set renders an explicit "无研究标签" chip, never a
...
export function researchTagChipsHtml(tags) {
  ...
  // Label states what the value holds (integration finding F-009): the
  // qf.research_tags.v1 key stays `decay_horizon_days`, but for factor
  // subjects the backend populates it from FactorDefinition.horizon_days —
  // the holding/signal horizon — because no measured decay estimate exists
  // (research_tags.py, FP-4). A "decay" label here would claim a decay
  // parameter the run never had.
  if (tags.decay_horizon_days != null) chips.push(chip('horizon ' + tags.decay_horizon_days + 'd'));
